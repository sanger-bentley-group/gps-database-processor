...
def check_meta_table(df_meta, table):
...
def check_qc_table(df_qc, table):
...
def check_country(df, column_name, table):
    expected = {'ARGENTINA', 'BANGLADESH', 'BELARUS', 'BENIN', 'BOTSWANA', 'BRAZIL', 'BULGARIA', 'CAMBODIA', 'CAMEROON', 'CANADA', 'CENTRAL AFRICAN REPUBLIC', 'CHINA', 'CROATIA', 'COLOMBIA', 'CZECH REPUBLIC', 'DOMINICAN REPUBLIC', 'DR CONGO', 'ECUADOR', 'EGYPT', 'ETHIOPIA', 'FRANCE', 'GHANA', 'GUATEMALA', 'HUNGARY', 'INDIA', 'INDONESIA', 'IRELAND', 'ISRAEL', 'IVORY COAST', 'KENYA', 'KUWAIT', 'LATVIA', 'LEBANON', 'LITHUANIA', 'MALAWI', 'MALAYSIA', 'MONGOLIA', 'MOROCCO', 'MOZAMBIQUE', 'NEPAL', 'NETHERLANDS', 'NEW ZEALAND', 'NIGER', 'NIGERIA', 'OMAN', 'PAKISTAN', 'PAPUA NEW GUINEA', 'PERU', 'POLAND', 'QATAR', 'RUSSIAN FEDERATION', 'SENEGAL', 'SLOVENIA', 'SOUTH AFRICA', 'SPAIN', 'SWEDEN', 'TAIWAN', 'TANZANIA', 'THAILAND', 'THE GAMBIA', 'TOGO', 'TRINIDAD AND TOBAGO', 'TURKEY', 'UNITED STATES', 'VIETNAM', 'WEST AFRICA', '_', 'UNKNOWN'}
    check_case(df, column_name, table)
...
def check_err(df, column_name, table):
    check_case(df, column_name, table)
    check_regex(df, column_name, table, pattern=r'^(NOTFOUND|ERR[0-9]{6,7})$')


# Check column values are in valid ERS format only
def check_ers(df, column_name, table):
    check_case(df, column_name, table)
    check_regex(df, column_name, table, pattern=r'^ERS[0-9]{6,7}$')
