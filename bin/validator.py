...
def check_age_years(df, column_name, table):
    check_regex(df, column_name, table, allow_empty=True, float_range=(0, 130), no_alphabet_only_numeric=True)
...
def check_in_silico_serotype(df, column_name, table):
    check_case(df, column_name, table)
    check_regex(df, column_name, table, absolute=False, pattern=r'^(?!0)[0-9]{1,2}[A-Z]?$')
